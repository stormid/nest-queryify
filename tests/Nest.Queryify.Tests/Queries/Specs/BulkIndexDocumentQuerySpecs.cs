...
{
    public class BulkIndexDocumentQuerySpecs : QuerySpec<IBulkResponse>
    {
        public BulkIndexDocumentQuerySpecs(ElasticClientQueryObjectTestFixture fixture) : base(fixture)
        {
        }
        ...
        protected override void AssertExpectations()
        {
            ...
            Fixture.ShouldUseUri(new Uri("http://localhost:9200/_bulk?refresh=false"));
        }

        protected override ElasticClientQueryObject<IBulkResponse> Query()
        {
            var list = Enumerable.Repeat(new Person(), 10);
            return new BulkIndexDocumentQuery<Person>(list);
        }
    }
}